...
class TestLogin:
    ...
    async def test_login_nonexistent_user(self, client: AsyncClient):
        ...
        response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "nonexistent@example.com", "password": "AnyPassword1"},
        )
    ...
    async def test_login_missing_username(self, client: AsyncClient):
        ...
        response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login", data={"password": "Password123"}
        )
...
class TestTestToken:
    ...
    async def test_token_missing(self, client: AsyncClient):
        ...
        response = await client.post(f"{settings.API_V1_PREFIX}/auth/test-token")
    ...
    async def test_token_for_deleted_user(
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "deleteme@example.com", "password": "Password123"},
        )
        token = login_response.json()["access_token"]
...
class TestAuthenticationFlow:
    ...
    async def test_complete_auth_flow(
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "fullflow@example.com", "password": "Password123"},
        )
