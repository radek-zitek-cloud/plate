...
class TestDependencies:
    ...
    async def test_get_db_dependency(
        ...
        user_in = UserCreate(
            email="dbtest@example.com", username="dbtestuser", password="Password123"
        )
    ...
    @pytest.mark.asyncio
    async def test_oauth2_scheme_extracts_token(
        self, client: AsyncClient, db_session: AsyncSession
    ):
        """Test that OAuth2 scheme correctly extracts Bearer token."""
        # Create and login
        user_in = UserCreate(
            email="oauth@example.com", username="oauthuser", password="Password123"
        )
    ...
    async def test_get_current_user_dependency(
        ...
        # Create user
        user_in = UserCreate(
            email="current@example.com", username="currentuser", password="Password123"
        )
    ...
    async def test_get_current_user_token_for_nonexistent_user(
        ...
        # Create user
        user_in = UserCreate(
            email="deleted@example.com", username="deleteduser", password="Password123"
        )
        user = await crud.user.create(db_session, obj_in=user_in)
    ...
    async def test_get_current_active_user_dependency(
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "active@example.com", "password": "Password123"},
        )
        token = login_response.json()["access_token"]
    ...
    async def test_get_current_superuser_dependency(
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "super@example.com", "password": "Password123"},
        )
        token = login_response.json()["access_token"]
    ...
    async def test_dependency_chain_execution_order(
        ...
        # Create user
        user_in = UserCreate(
            email="chain@example.com", username="chainuser", password="Password123"
        )
        await crud.user.create(db_session, obj_in=user_in)
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "chain@example.com", "password": "Password123"},
        )
        token = login_response.json()["access_token"]
