...
async def normal_user_token(client: AsyncClient, db_session: AsyncSession) -> str:
    ...
    response = await client.post(
        ...
        data={"username": "normaluser@example.com", "password": "Password123"},
    )
...
async def superuser_token(client: AsyncClient, db_session: AsyncSession) -> str:
    ...
    response = await client.post(
        f"{settings.API_V1_PREFIX}/auth/login",
        data={"username": "superuser@example.com", "password": "Password123"},
    )
    return response.json()["access_token"]
...
class TestUserSignup:
    ...
    async def test_signup_duplicate_email(
        ...
        # Create first user
        user_in = UserCreate(
            email="duplicate@example.com", username="firstuser", password="Password123"
        )
        await crud.user.create(db_session, obj_in=user_in)
    ...
    async def test_signup_duplicate_username(
        ...
        # Create first user
        user_in = UserCreate(
            email="first@example.com", username="duplicateuser", password="Password123"
        )
        await crud.user.create(db_session, obj_in=user_in)
...
class TestReadUserMe:
    ...
    async def test_read_user_me_success(
        self, client: AsyncClient, normal_user_token: str
    ):
...
class TestUpdateUserMe:
    ...
    @pytest.mark.asyncio
    async def test_update_user_me_username(
        self, client: AsyncClient, normal_user_token: str
    ):
    ...
    async def test_update_user_me_password(
        ...
        login_response = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "changepw@example.com", "password": "OldPassword1"},
        )
        ...
        new_login = await client.post(
            f"{settings.API_V1_PREFIX}/auth/login",
            data={"username": "changepw@example.com", "password": "NewPassword123"},
        )
    ...
    async def test_update_user_me_unauthorized(self, client: AsyncClient):
        ...
        response = await client.put(
            f"{settings.API_V1_PREFIX}/users/me", json={"email": "newemail@example.com"}
        )
...
class TestReadUserById:
    ...
    async def test_read_user_by_id_unauthorized(
        ...
        response = await client.get(f"{settings.API_V1_PREFIX}/users/{user.id}")
...
class TestReadUsers:
    ...
    @pytest.mark.asyncio
    async def test_read_users_as_normal_user(
        self, client: AsyncClient, normal_user_token: str
    ):
    ...
    async def test_read_users_unauthorized(self, client: AsyncClient):
        ...
        response = await client.get(f"{settings.API_V1_PREFIX}/users/")
...
class TestDeleteUser:
    ...
    async def test_delete_user_unauthorized(
        ...
        response = await client.delete(f"{settings.API_V1_PREFIX}/users/{user.id}")
